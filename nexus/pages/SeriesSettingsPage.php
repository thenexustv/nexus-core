...
class SeriesSettingsPage extends SettingsPage {
	...
	public function __construct($series_id) {
		...
		$settings = \Nexus\Settings::get_instance()->get_series_settings($series_id);

		parent::__construct($settings);

		$this->series = \Nexus\Series::get_by_series($series_id);
	...
	public function render_text_field($key) {
		$template = sprintf('<input type="text" name="%s" id="%s" value="%s" class="widefat" />',
	...
	public function render_series_default_hosts($key) {
		...
		foreach($person_ids as $person_id) {
			$person = \Nexus\Person::factory($person_id);
			$data[] = array(
